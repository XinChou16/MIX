...
class Store {
  constructor(options = {}) {
    ...
    installModules(this, state, [], options);
  ...
  watch(getter, callback, options) {
    return this._watcherVM.$watch(() => getter(this.state), callback, options);
  }
}
...
function resetStore(store) {
  ...
  // reset root store
  installModules(store, state, [], store._options, true);

  // reset runtime store
  Object.keys(store._runtimeModules).forEach(key => {
    installModules(store, state, key.split('.'), store._runtimeModules[key], true);
  });
  // reset vm
