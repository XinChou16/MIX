 ...
 * Promise test
 */
const MyPromise = require('./promise-5');
...
promise({ value: 12})//.then(console.log);

// describe('demo', () => {
//   it('666', () => {
//     expect('666').toEqual('666');
//   });
// });
...
describe("catch", () => {
  ...
  it("with chaining2", () => {
    return promise({ value: 3 })
      ...
      .then(null, v => expect(v).toEqual(12))
  })
})
...
describe("finally", () => {
  ...
  it("with chaining", () => {
    ...
    const failPromise = promise({ fail: true })
      .then(v => v)
      .finally(checkFunc)
    const failPromise2 = promise({ fail: true })
      .then(null, v => v)
      .finally(checkFunc)
    return Promise.allSettled([successPromise, failPromise, failPromise2])
  })
})
